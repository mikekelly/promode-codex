...
**Promode already runs this flywheel once.** The `AGENTS.md`-rooted agent-knowledge graph *is* its first instance in Codex: a knowledge node is a crystallised discovery about the *repo*; "orient before you act" is the where-am-I check. Everything below aims the same loop at a *running app* instead of a codebase.
</the-flywheel>
...
- **The UI tier is verification-only.** It is the `promode_verifier`'s slow tier, exercised surgically. It is **never a debugging loop** — "system tests are for verification, not debugging" still holds; the headless seam is precisely what keeps the *bulk* of feedback fast.
