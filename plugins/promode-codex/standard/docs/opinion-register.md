...
| M1 `opinions-not-tutorials` | Promode prompts instill taste, constraints, and non-derivable doctrine; they do not teach the model generic software engineering. | custom agents, managing docs |
...
| M8 `preserve-project-ownership` | `AGENTS.md` and non-Promode `.codex/` files are project-owned. Promode may guide or sync Promode-owned files, not silently rewrite project guidance. | managing skill, installer |
...
| O2 `delegation-consent` | Codex does not use silent fire-and-forget delegation. Use subagents only when the user asked for Promode, delegation, parallel work, or approved it. | activate skill |
...
| O6 `disjoint-write-scopes` | Parallel coding agents get non-overlapping write scopes and must not revert unrelated edits. | activate skill, implementer |
...
| P2 `tdd-non-negotiable` | For code changes, write or identify a failing behavioral test before implementation whenever feasible. | activate skill, implementer, reviewer |
| P3 `tests-are-documentation` | Tests are executable behavioral documentation and outrank prose that claims different behavior. | activate skill, implementer, reviewer |
...
| P6 `crystallise-discovery` | Useful discovery becomes deterministic checks, scripts, maps, tests, or runbooks; it should not remain only prose. | activate skill, reviewer, verifier |
| P7 `behavioral-authority` | Verified behavior controls: passing tests, failing tests, explicit specs, code, then external docs. | reviewer, implementer |
| P8 `stay-on-task-flag-rest` | Fix the assigned problem, flag adjacent issues for triage, and avoid opportunistic scope creep. | implementer, debugger |
...
| K3 `decision-nodes` | Hard-to-reverse or surprising decisions earn a node that records what was decided and why. | activate skill, implementer, product designer |
...
| K5 `one-idea-one-home` | Keep one canonical home for durable knowledge and link to it instead of forking prose across the corpus. | audit skill, assumptions |

## Testing and verification

| id | statement | homes |
| --- | --- | --- |
| T1 `one-test-at-a-time` | Do not batch a pile of tests and then code. Drive one failing behavior to green, then continue. | implementer |
| T2 `red-for-right-reason` | A new test must fail because the intended behavior is missing, not because of setup noise or a bad assertion. | implementer |
| T3 `outside-in-behavior` | Prefer tests that express user-visible or externally observable behavior over implementation coupling. | implementer, reviewer |
| T4 `operator-seam-bulk-below-ui` | Keep most acceptance coverage below the GUI through a real operator seam; use the GUI tier only for behavior that manifests there. | activate skill, discovery-to-determinism, verifier |
...
| T6 `seam-changes-are-design` | Extending a shared operator seam can be implementation work; reshaping its boundary is an architectural decision for the parent agent. | implementer, activate skill |
| T7 `crystallised-failure-triage` | A failing deterministic check asks for classification first: flake, intended change, or regression. | activate skill, debugger |
...
| id | statement | homes |
| --- | --- | --- |
| R1 `two-axis-review` | Review spec fit and engineering standards as independent axes. | reviewer |
| R2 `distrust-narration` | Commit messages, task docs, and summaries are claims; review the diff and tests directly. | reviewer |
| R3 `reviewer-does-not-substitute-for-tests` | Reviewers inspect correctness, design, and test quality. They do not replace the implementer's test loop. | reviewer, activate skill |
| R4 `crisp-verdicts` | Judging agents return clear verdicts such as APPROVED/REWORK or PASS/FAIL. | reviewer, verifier |
...
| id | statement | homes |
| --- | --- | --- |
| PD1 `skeptical-default` | Most feature requests are solutions looking for problems; challenge the problem, user, and need before adding surface area. | product designer |
| PD2 `defaults-over-settings` | Prefer good defaults and constraints over extra configuration. | product designer |
| PD3 `user-needs-are-claims` | Workflows and use cases are claims about real people; cite evidence or flag the assumption. | product designer, activate skill |
...
| A1 `entity-model-highest-stakes` | Entity/domain model decisions are high leverage and expensive to unwind; treat them as hard-to-reverse. | product designer, activate skill |
| A2 `reversibility-weighted-depth` | Spend design depth on one-way doors; decide reversible details quickly and record the distinction when it matters. | activate skill |
...
| C4 `discovery-to-determinism-skill` | Operator seams and UI state graphs are important enough to have their own reusable mechanics skill. | discovery-to-determinism |
| C5 `promode-audit-skill` | Alignment with Promode doctrine is a separate audit job with concrete evidence, not a vague self-assessment. | promode-audit |
